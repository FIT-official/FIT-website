...
export const FILAMENT_MATERIALS = Object.freeze([
  { value: 'pla', label: 'PLA' }, { value: 'pla_matte', label: 'PLA Matte' },
  { value: 'petg', label: 'PETG' }, { value: 'asa', label: 'ASA' },
  { value: 'abs', label: 'ABS' }, { value: 'tpu', label: 'TPU' },
])

export const DEFAULT_SIMPLE_SELECTION = Object.freeze({ purpose: '', material: 'plastic', filament: 'pla', colour: 'Jade White' })
export const DEFAULT_EDITOR_PRINT_SETTINGS = Object.freeze({
  layerHeight: 0.2, initialLayerHeight: 0.2, materialType: 'plastic', filamentType: 'pla', wallLoops: 2,
...
export function purposeFromPrintSettings(settings = {}) {
  return Object.keys(PURPOSE_PRESETS).find(purpose => {
    ...
    return Object.keys(DEFAULT_EDITOR_PRINT_SETTINGS).filter(key => !['materialType', 'filamentType'].includes(key))
...
export function mapPurposeToConfiguration(selection = {}, colours = DEFAULT_PRINT_COLOURS) {
  ...
  const filament = FILAMENT_MATERIALS.some(m => m.value === selection.filament) ? selection.filament : 'pla'
  ...
  return {
    purpose,
    generic: { strength: preset.strength, quality: preset.quality, colour, material, filament },
    printSettings: { ...DEFAULT_EDITOR_PRINT_SETTINGS,
      ...
      materialType: material, filamentType: filament },
    colourHex: colours.find(entry => entry.name === colour && (!entry.filament || entry.filament === filament))?.hex || mapped.colourHex,
  }
}
...
export function restorePrintConfiguration(configuration = {}, colours = DEFAULT_PRINT_COLOURS) {
  ...
  const colourName = savedColours.length === 1
    ? colours.find(colour => colour.hex?.toLowerCase() === savedColours[0]?.toLowerCase()
      && (!colour.filament || colour.filament === (printSettings.filamentType || 'pla')))?.name : null
  return {
    printSettings,
    selection: { purpose, material: printSettings.materialType,
      filament: printSettings.filamentType || generic.filament || 'pla',
...
export function applySimpleSelection(current, selection, field, meshNames = [], colours = DEFAULT_PRINT_COLOURS) {
  ...
  return {
    printSettings: field === 'purpose' ? mapped.printSettings
      : { ...current.printSettings, materialType: selection.material, filamentType: selection.filament || 'pla' },
    selection,
    meshColors: ['colour', 'filament'].includes(field) && mapped.colourHex
