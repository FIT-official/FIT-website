...
const previewOwnedGeometry = new WeakSet();

function usableNormal(normals, index) {
    if (!normals || normals.itemSize !== 3 || index >= normals.count) return false;
    const x = normals.getX(index), y = normals.getY(index), z = normals.getZ(index);
    const lengthSquared = x * x + y * y + z * z;
    return Number.isFinite(lengthSquared) && lengthSquared > 1e-20;
}

function geometryForPresentation(geometry) {
    const positions = geometry?.getAttribute('position');
    if (!positions?.count) return geometry;
    const originalNormals = geometry.getAttribute('normal');
    let repair = !originalNormals || originalNormals.itemSize !== 3 || originalNormals.count !== positions.count;
    for (let index = 0; !repair && index < positions.count; index++) repair = !usableNormal(originalNormals, index);
    if (!repair) return geometry;

    // Some STL exporters write zero facet normals. Repair only the display
    // clone: quote geometry and valid authored normals (including hard edges)
    // remain unchanged. Degenerate faces still have no surface to illuminate.
    const displayed = geometry.clone();
    displayed.deleteAttribute('normal');
    displayed.computeVertexNormals();
    const normals = displayed.getAttribute('normal');
    for (let index = 0; index < normals.count; index++) {
        if (usableNormal(originalNormals, index)) {
            normals.setXYZ(index, originalNormals.getX(index), originalNormals.getY(index), originalNormals.getZ(index));
        }
    }
    previewOwnedGeometry.add(displayed);
    return displayed;
}
...
export function preparePrintModel(source, { fileName = '', upAxis = 'auto', modelScale = 1,
    ...
    model.traverse(child => {
        ...
        if (child.isMesh) {
            if (!geometries.has(child.geometry)) geometries.set(child.geometry, geometryForPresentation(child.geometry));
...
export function disposePrintModel(root) {
    // Dispose repaired display clones once; shared source geometry may also
    // drive quotes or another view and is never owned by this preview.
    const disposed = new Set();
    root.traverse(child => {
        if (!child.isMesh) return;
        (Array.isArray(child.material) ? child.material : [child.material]).forEach(material => material?.dispose());
        if (previewOwnedGeometry.has(child.geometry) && !disposed.has(child.geometry)) {
