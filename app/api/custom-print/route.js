...
// GET /api/custom-print?requestId=...
...
// POST multipart/form-data to create a new request with uploaded model
export async function POST(request) {
  ...
  const title = formData.get('title') || 'Custom print request'
  ...
  const key = `custom-print/${user.id}/${requestId}/${file.name}`
  ...
  const doc = await CustomPrintRequest.create({
    requestId,
    userId: user.id,
    ...
    statusHistory: [
      {
        status: 'pending_config',
        note: 'Request created and awaiting configuration in editor.',
      },
    ],
  })
