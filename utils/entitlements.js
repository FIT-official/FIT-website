...
export const TIERS = { FREE: 'free', STANDARD: 'standard', PRO: 'pro', LEGACY: 'legacy' }
...
export async function getEntitlements({ role, planId, status, priceId, priceIds } = {}) {
    ...
    const tier = ['active', 'trialing'].includes(status) && ['standard', 'pro', 'legacy'].includes(selected) ? selected : 'free'
