...
describe('Plan catalogue', () => {
    it('returns Free plus both paid tiers with monthly and annual billing', async () => {
        ...
        expect(plans.map(p => [p.id, p.interval, p.amount, p.available])).toEqual([
            ['free', 'month', 0, true], ['student', 'month', 0, true], ['standard', 'month', 39, true], ['standard', 'year', 390, true],
            ['pro', 'month', 99, true], ['pro', 'year', 990, true],
        ])
        expect(plans[3]).toMatchObject({ monthlyEquivalent: 32.5, annualSavings: 78, limits: { products: 50, monthlyPrintRequests: 100 } })
        expect(plans[5]).toMatchObject({ monthlyEquivalent: 82.5, annualSavings: 198, limits: { products: 5000, monthlyPrintRequests: 500 } })
    })
    it.each([
        ...
        expect(plans[3].available).toBe(true)
    })
    it.each([
        ...
        const { plans } = await (await GET()).json()
        expect(plans[3]).toMatchObject({ available: false, priceId: null })
    ...
    it('keeps monthly options available when annual IDs have not been configured', async () => {
        ...
        const { plans } = await (await GET()).json()
        expect(plans.map(p => p.available)).toEqual([true, true, true, false, true, false])
    ...
    it('rejects a reused price ID across billing intervals without hiding unrelated tiers', async () => {
        ...
        const { plans } = await (await GET()).json()
        expect(plans.map(p => p.available)).toEqual([true, true, false, false, true, true])
    })
    it('only hides the affected billing option when its Stripe lookup fails', async () => {
        ...
        const { plans } = await (await GET()).json()
        expect(plans.map(p => p.available)).toEqual([true, true, true, true, true, false])
    })
    it('keeps free registration independent of Stripe configuration and outages', async () => {
        vi.stubEnv('STRIPE_SECRET_KEY', '')
        const { plans } = await (await GET()).json()
        expect(plans.map(p => p.available)).toEqual([true, true, false, false, false, false])
